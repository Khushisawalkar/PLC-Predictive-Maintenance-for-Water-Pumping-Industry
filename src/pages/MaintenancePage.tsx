    ...
    <div className="flex flex-col gap-4 h-full">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-[#070d13] border border-[#1e2d3d] rounded p-4 flex flex-col gap-2">
          <div className="text-sm font-semibold text-[#8a9aaa] flex justify-between items-center tracking-wide">
          ...
          <div className="flex items-end gap-2">
            ...
            <span className="text-[#8a9aaa] text-sm mb-1 font-semibold">REMAINING</span>
          </div>
        </div>

        <div className="bg-[#070d13] border border-[#1e2d3d] rounded p-4 flex flex-col gap-2 md:col-span-2">
          <div className="text-sm font-semibold text-[#8a9aaa] flex items-center gap-2 tracking-wide">
            <Cpu size={16} /> AI DIAGNOSTIC SUMMARY
          </div>
          <p className="text-[#c0d0e0] text-sm leading-relaxed mt-1">
            {overallHealth < 0.6 
          ...
          {predictions.map((pred, i) => (
            <div key={i} className="border border-[#1e2d3d] bg-[#0a1118] rounded p-4 flex flex-col md:flex-row items-start md:items-center gap-4 md:gap-6">
              <div className="flex-1 w-full">
                <div className="text-sm font-bold text-[#c0d0e0] mb-1 tracking-wide">{pred.component.toUpperCase()}</div>
                <div className="text-xs text-[#8a9aaa]">{pred.recommendation}</div>
              </div>
              
              <div className="flex flex-col items-start md:items-end w-full md:w-48">
                <div className="text-xs font-semibold text-[#8a9aaa] mb-1 tracking-wide">FAILURE PROBABILITY</div>
              ...
              <div className="flex flex-col items-start md:items-end w-full md:w-32 border-t md:border-t-0 md:border-l border-[#1e2d3d] pt-3 md:pt-0 md:pl-6">
                <div className="text-xs font-semibold text-[#8a9aaa] mb-1 tracking-wide">EST. LIFESPAN</div>
                <div className={`text-xl font-mono font-bold ${pred.estimatedDaysRemaining < 60 ? 'text-amber-400' : 'text-emerald-400'}`}>
